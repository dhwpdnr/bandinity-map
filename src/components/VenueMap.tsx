...
export function VenueMap({ venues, selectedId, onSelectVenue }: VenueMapProps) {
  ...
  if (error) {
    return (
      <div className="flex h-full min-h-[280px] items-center justify-center rounded-[20px] border border-white/12 bg-[rgba(11,14,20,0.92)] p-4 text-center text-sm text-zinc-300">
  ...
  if (loading) {
    return (
      <div className="flex h-full min-h-[280px] items-center justify-center rounded-[20px] border border-white/12 bg-[rgba(11,14,20,0.92)]">
        <span className="text-sm text-zinc-300">지도 로딩 중...</span>
      </div>
    );
  }

  return (
    <div className="relative h-full min-h-[280px] w-full overflow-hidden rounded-[20px] border border-white/10 bg-[var(--surface-map)] p-1.5 shadow-[0_12px_28px_-24px_rgba(0,0,0,0.72)]">
      <Map
        ref={mapRef}
        center={center}
        level={level}
          ...
          {venues.map((venue) => {
            ...
            return (
              ...
              >
                {isSelected && (
                  <PopupContent>
                    <div className="rounded-[10px] border border-white/12 bg-[rgba(11,14,20,0.96)] px-2.5 py-1.5 shadow-[0_10px_24px_-20px_rgba(0,0,0,0.8)]">
                      <p className="max-w-[160px] truncate text-sm font-medium text-zinc-100">
                        {venue.name}
                      </p>
                      <Link
                        href={`/venues/${venue.id}`}
                        className="mt-0.5 block text-xs text-primary-400 hover:text-primary-300"
